        ...
        for (unsigned int meshIndex = 0; meshIndex < meshCount; meshIndex++) {
            for(unsigned int triangleIndex = 0; triangleIndex < meshes[meshIndex].vertexCount / 3; triangleIndex++) {
                ...
                float4 v2 = meshes[meshIndex].vertices[triangleIndex * 3 + 2];
								//std::cout << "/* message */" << meshes[meshIndex].vertexCount / 3 << '\n';
...
__global__ void device_initFrameBuffer(int* pointer) {
...
__global__ void device_initDepthBuffer(int* pointer) {
	//int* depthBuffer = new int[*width * *height];
	// for(unsigned int i = 0; i < width * height; i++) {
	// 		depthBuffer[i] = 16777216;
	// }
}
...
std::vector<unsigned char> rasteriseGPU(std::string inputFile, unsigned int width, unsigned int height, unsigned int depthLimit) {
    ...
    std::cout << "Loading '" << inputFile << "' file... " << std::endl;
		...
		std::cout << "cudaGetDeviceCount: " << deviceCount << std::endl;

		// task 2b)
		cudaDeviceProp deviceProp;
		checkCudaErrors(cudaGetDeviceProperties(&deviceProp, 0));
		std::cout << "cudaGetDeviceProperties: " << deviceProp.name << std::endl;

		// task 2c)
		checkCudaErrors(cudaSetDevice(0));

		// task 4a)
		int* frame_pointer, depth_pointer;
		size_t frame_size = width * height * 4 * sizeof(int);
		size_t depth_size = width * height * sizeof(int);
		checkCudaErrors(cudaMalloc((void**) &frame_pointer, frame_size));
		checkCudaErrors(cudaMalloc((void**) &depth_pointer, depth_size));

		// task 4b)
		dim3 grid(2, 1, 1);
		dim3 block(3, 1, 1);
		device_initFrameBuffer<<<1, 32>>>(frame_pointer);
		checkCudaErrors(cudaDeviceSynchronize());

		//checkCudaErrors(cudaMemcpy(frame_pointer, frameBuffer, frame_size, cudaMemcpyHostToDevice));
		//checkCudaErrors(cudaMemcpy(depth_pointer, depthBuffer, depth_size, cudaMemcpyHostToDevice));
